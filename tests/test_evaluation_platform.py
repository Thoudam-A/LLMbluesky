...
class PortablePlatformTests(unittest.TestCase):
    ...
    def test_dynamic_separation_formula_uses_consistent_html_subscripts(self):
        text = (ROOT / "evaluation_platform/static/index.html").read_text(encoding="utf-8")
        self.assertIn("R<sub>sep</sub> = N<sub>safe</sub> / N<sub>eligible</sub>", text)
        self.assertIn("R<sub>acc</sub> = N<sub>acc</sub> / (N<sub>acc</sub> + N<sub>rej</sub>)", text)
        self.assertNotIn("Rₛₑₚ = Nₛₐfₑ / Nₑₗᵢgᵢbₗₑ", text)
        self.assertNotIn("Rₐcc = Nₐcc / (Nₐcc + Nᵣₑⱼ)", text)
