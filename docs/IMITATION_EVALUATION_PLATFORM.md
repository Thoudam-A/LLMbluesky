...
当前已经注册管制指令模仿精度、动态间隔调整成功率、管制指令执行接受度、
管制指令自主生成响应时间和管制意图智能理解准确率。每项指标拥有独立manifest和评分器，
共享任务状态、证据归档和结果展示外壳。“已实现”不等于本机正式输入已经配置完成。
...
模仿精度支持`archived_result`、`rescore_existing`和`full_replay`三种模式；
意图理解指标支持基于冻结预测结果重新评分；三项H-PPO指标支持读取既有运行日志离线评分。
Web页面通过真实API加载指标JSON，不写死正式数值。当前五项评分器均已通过合成测试，
正式评估仍需配置本地数据、模型输出或H-PPO日志。
