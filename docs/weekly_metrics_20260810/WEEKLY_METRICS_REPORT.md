...
- 管制员指令模仿精度（系统指令精度口径）：**48.72%**；
- 管制意图动作识别准确率：**87.14%**；
- 模仿宏平均召回 / 意图类型识别F1：**53.68% / 80.34%**；
- 意图类型精确率 / 召回率：**71.71% / 91.33%**；
- 完整意图框架准确率：**35.13%**。

> 汇报时必须保留口径名称：48.72%是系统输出指令中成功匹配历史指令的比例；87.14%是参考意图动作槽位命中率，不是完整意图框架准确率。完整框架口径当前为35.13%。

## 可视化

### 指标评估平台：管制员指令模仿精度

![指标评估平台—管制员指令模仿精度](screenshots/platform_imitation_precision.jpg)

### 指标评估平台：管制意图动作识别准确率

![指标评估平台—管制意图动作识别准确率](screenshots/platform_intent_action_accuracy.jpg)

以上截图由评估平台的本周汇报模式直接加载固化结果生成，可通过下列地址复现：
